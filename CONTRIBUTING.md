...
Three suites, all offline and free. The fake reader replays the fixture
ground truth, so no API key is needed for any of them:
...
Playwright boots both servers itself from `web/playwright.config.ts`, which
reads the repo-root `.env`. Copy `.env.example` to `.env` with
`READER_PROVIDER=fake` and any non-empty tokens before running it.

### Adding a fixture label

A fixture is one image plus its ground truth, and the ground truth is generated,
not hand-edited:
...
3. Run `cd api && uv run python scripts/build_fixtures.py`. It renders the image
   ...
   files) and `expectations.json` (what a perfect reader sees, plus `quality`,
